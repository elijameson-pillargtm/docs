...
PILLAR's MCP surface spans **seven tiers** (Core + A–F + G), covering every layer of the revenue architecture. The Vertical Intelligence tier (Tier G, **48 tools live**) is the largest single-tier addition since the original launch — it carries PILLAR's canonical district + federal-program datasets that horizontal Revenue AI platforms structurally cannot answer.

<Note>
**Coverage as of April 2026**: Tier G is backed by **51 jurisdictions** (50 states + DC + federal) and **26 federal datasets** (8 IPEDS components + 8 Higher Ed sources + 10 K-12 sources), exposed through **48 MCP tools** (24 original + 24 Round 8 additions). The schema, ingest pipeline, MCP wrappers, and 550+ build-time-enforced Guarantees are runtime-truth — every commit blocks merge unless the canonical-shape validators (`G-X-31` through `G-X-40`) accept every row landing in the 26 new federal-data tables. All Round 8 tools (Scorecard Field-of-Study, FSA CDR/GE/HCM/distress-score, SHEEO SHEF, NC-SARA, Carnegie 2025, IPEDS HR/ADM/AY/AL/EF-CIP, CCD School Universe, EDGE Locale, CRDC ×2, OSEP IDEA-B, McKinney-Vento, Title III, Migrant Ed, Perkins V, NSLP-CEP, NIEER) are live and MCP-callable today.
</Note>
...
### Tier G — Vertical Intelligence (48 tools live)
