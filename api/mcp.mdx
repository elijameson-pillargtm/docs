...
### Tier G — Vertical Intelligence (9 tools)

<Info>
**The competitive moat.** Horizontal revenue platforms (Salesforce, HubSpot, Gong, Clari) cannot ship these tools because they don't maintain federal education datasets, accreditor calendars, or per-state procurement timing. PILLAR does — the Tier G surface answers questions the customer's own procurement office and CFO operate in.
</Info>

K-12 + HigherEd specific tools backed by maintained federal datasets (IPEDS, NCES CCD, EDFacts, F-33 district finance), accreditor cycles, state fiscal calendars, and Title program formulas.
...
**K-12 — Federal Title dollars + district demographics (NCES LEAID-keyed)**
