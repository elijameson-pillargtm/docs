...
PILLAR's MCP surface spans **seven tiers** (Core + A–F + G), covering every layer of the revenue architecture. The Vertical Intelligence tier (Tier G, 24+ tools live + expanding) is the largest single-tier addition since the original launch — it carries PILLAR's canonical district + federal-program datasets that horizontal Revenue AI platforms structurally cannot answer.

<Note>
**Coverage as of April 2026**: Tier G is now backed by **51 jurisdictions** (50 states + DC + federal) and **26 federal datasets** (8 IPEDS components + 8 Higher Ed sources + 10 K-12 sources). The schema, ingest pipeline, and 462+ build-time-enforced Guarantees are runtime-truth — every commit blocks merge unless the canonical-shape validators (`G-X-31` through `G-X-40`) accept every row landing in the 26 new federal-data tables. Tool surfaces for the latest federal datasets (Scorecard Field-of-Study, FSA CDR/GE/HCM, SHEEO SHEF, Carnegie 2025, OSEP IDEA-B, etc.) ship in the next MCP release; the underlying data is queryable via direct Postgres access today.
</Note>
...
### Tier G — Vertical Intelligence (24 tools live, 30+ in next release)
