...
<AccordionGroup>
  ...
  <Accordion title="What is Drafter?">
    **Drafter** is the in-app AI agent layer, mounted globally across every authenticated PILLAR page as a floating chat widget. Ask any GTM question — "why does Houston ISD have its current renewal risk score?", "what's our biggest pipeline risk this quarter?", "simulate NRR going from 108% to 115%" — and Drafter picks the right subset of MCP tools, pulls evidence, and returns a specific answer. Every run writes to an audit table for the approval gate and flywheel learning. Drafter and the external MCP server share the same 70+ MCP tool catalog, so behavior is consistent across surfaces.
  </Accordion>
