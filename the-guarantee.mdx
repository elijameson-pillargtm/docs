...
**Runtime-truth status (April 2026)**: 51 jurisdictions covered (50 states + DC + federal); 26 federal datasets ingested with 890,000+ canonical rows ready for live upsert across 35 ingest scripts; **52 MCP tools** in Tier G (all live and queryable). Per-district coverage is at 51/51 jurisdictions for assessment proficiency (5.03M cells across ~19,700 LEAs), cohort graduation (391k cells), accountability status (24k cells), and engagement/chronic absenteeism (104k cells) — per-state DOE deep ingest for 27+ states at recent-year grain plus federal EDFacts SY 2020-21 backfill closing the long tail. 550 Guarantee tests pass on every commit; the schema, ingest pipeline, MCP wrappers, and canonical-shape validators are all enforced — every commit blocks merge unless every row landing in the 26 new federal-data tables passes its `G-X-31` through `G-X-40` validator.
