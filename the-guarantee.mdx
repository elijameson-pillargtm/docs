---
title: 'The Guarantee'
description: "PILLAR's correctness commitment: every behavior we promise is enforced by an automated test that runs on every release. 119 named invariants across 18 categories, 4,100+ tests on every commit, all green on every build. The largest category — Vertical Intelligence — covers PILLAR's translation of all 50 state Departments of Education + DC + 33 federal datasets into a unified schema."
icon: 'shield-check'
---
...
**Current count: 118 entries** spanning ten domains (scoring, vertical intelligence, configuration, ingestion, UI, tenancy, contract, signals, validation, ops). The Vertical Intelligence domain (`SPEC-VERTICAL-*`) is the fastest-growing surface at **46 entries**, reflecting the multi-layer reconciliation work that closes the gap between "PILLAR canonicalizes all 50 states + DC + 33 federal datasets" and "PILLAR's per-district numbers reconcile to the state DOE's own published values" — and the new Round 8 federal-data canonical-shape layer that closes "is the federal-dataset row right?"
...
**Current count: 119 Guarantees** across 18 categories. **Vertical Intelligence (X) is the largest category at 54 Guarantees** (G-X-01 through G-X-56), reflecting the depth of the canonicalization + accuracy-verification layer that translates 50 state DOEs + DC + 33 federal datasets into a unified schema.
...
**Current count: 4,193 tests** running on every commit (1,849 in the Guarantee suite directly, ~2,340 broader unit/integration), plus Playwright UI tests against the public auth + unsubscribe surfaces.
...
- **Procurement sign-off in days, not quarters.** Hand your CTO or CISO the spec. They read 113 behaviors, not 113 pages of marketing.
